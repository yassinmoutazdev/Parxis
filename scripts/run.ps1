...
if ($Action -eq 'Stop') {
    ...
    $stoppedBackend = Stop-TrackedProcess -PidFile $BackendPidFile -Label 'backend'
    $stoppedFrontend = Stop-TrackedProcess -PidFile $FrontendPidFile -Label 'frontend'
...
try {
    ...
    # --- Frontend dependencies (only if not already installed) ---
